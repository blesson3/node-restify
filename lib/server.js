...
function Server (options) {
  ...
  this.useGreenlock = options.useGreenlock || false;
  this.greenlockConfig = options.greenlockConfig;
  ...
  if (options.http2 || this.useGreenlock) {
    ...
    if (this.useGreenlock) {
      if (!this.greenlockConfig) throw new Error('Greenlock config not specified');
      ...
      require('greenlock-express').init({
        ...
        maintainerEmail: options.greenlockConfig.maintainerEmail,
        cluster: false
      })
        .ready((glx) => {
          this.http2 = true;
          this.server = glx.http2Server();
        });
    } else {
      this.http2 = true;
